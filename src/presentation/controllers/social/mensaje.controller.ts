...
export class MensajeController {
  ...
  async crear(@Body() body: any, @Req() req: any) {
    try {
      ...
      const respuesta = await this.mensajeService.createOrUpdate(data);
  ...
  async actualizar(@Param('id') id: number, @Body() datos: any) {
    try {
      datos.id = Number(id);
      const respuesta = await this.mensajeService.createOrUpdate(datos);
      return new Respuesta('OK', Finalizado.OK, respuesta);
    } catch (error) {
      throw new HttpException(
        new Respuesta(error.message, Finalizado.FAIL),
  ...
  async eliminar(@Param('id') id: number) {
    try {
      const respuesta = await this.mensajeService.eliminar(Number(id));
      return new Respuesta('OK', Finalizado.OK, respuesta);
    } catch (error) {
      throw new HttpException(
        new Respuesta(error.message, Finalizado.FAIL),
  ...
  async mostrar(@Param('id') id: number) {
    try {
      const respuesta = await this.mensajeService.findOne({ id: Number(id) });
      return new Respuesta('OK', Finalizado.OK, respuesta);
    } catch (error) {
      throw new HttpException(
        new Respuesta(error.message, Finalizado.FAIL),
  ...
  async listar(@Query() params: any) {
    try {
      const respuesta = await this.mensajeService.listar(params);
      return new Respuesta('OK', Finalizado.OK, respuesta);
    } catch (error) {
      throw new HttpException(
        new Respuesta(error.message, Finalizado.FAIL),
