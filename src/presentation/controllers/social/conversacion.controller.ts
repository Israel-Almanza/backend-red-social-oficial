...
@Controller('social/conversacion')
export class ConversacionController {
  ...
  async crear(@Body() body: any, @Req() req: any) {
    try {
      ...
      const respuesta = await this.conversacionService.createOrUpdate(data);
  ...
  async actualizar(@Param('id') id: number, @Body() datos: any) {
    try {
      datos.id = Number(id);
      const respuesta = await this.conversacionService.createOrUpdate(datos);
      return new Respuesta('OK', Finalizado.OK, respuesta);
    } catch (error) {
      throw new HttpException(
        new Respuesta(error.message, Finalizado.FAIL),
  ...
  async eliminar(@Param('id') id: number) {
    try {
      const respuesta = await this.conversacionService.eliminar(Number(id));
      return new Respuesta('OK', Finalizado.OK, respuesta);
    } catch (error) {
      throw new HttpException(
        new Respuesta(error.message, Finalizado.FAIL),
  ...
  async mostrar(@Param('id') id: number) {
    try {
      const respuesta = await this.conversacionService.findOne({ id: Number(id) });
      return new Respuesta('OK', Finalizado.OK, respuesta);
    } catch (error) {
      throw new HttpException(
        new Respuesta(error.message, Finalizado.FAIL),
  ...
  @UseGuards(JwtAuthGuard) 
  // , PermissionGuard)
  // @Permissions('conversacion:listar')
  @Get()
  async listar(@Query() params: any) {
    try {
      const respuesta = await this.conversacionService.listar(params);
      return new Respuesta('OK', Finalizado.OK, respuesta);
    } catch (error) {
      throw new HttpException(
        new Respuesta(error.message, Finalizado.FAIL),
